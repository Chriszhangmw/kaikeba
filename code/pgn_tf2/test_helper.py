...
def beam_decode(model, batch, vocab, params):
    ...
    def decoder_one_step(enc_output, dec_input, dec_hidden, enc_extended_inp, batch_oov_len, enc_pad_mask,
        ...
        # 拿到top k个index 和 概率
        top_k_probs, top_k_ids = tf.nn.top_k(tf.squeeze(final_pred), k=params["beam_size"] * 2) #这里为什么乘以2
        # 计算log概率
    ...
    while steps < params['max_dec_len'] and len(results) < params['beam_size']:
        # 获取最新待使用的token
        latest_tokens = [h.latest_token for h in hyps] #latest_tokens表示最新的一个tocken
        ...
        #这里对应上周的代码： dec_input = tf.constant([vocab.word_to_id('[START]')] * batch_size)；dec_input = tf.expand_dims(dec_input, axis=1)
        ...
        #如果是初始步骤，那取第一个就行，如果不是，那么每搜索一部，都会有对应len(hyps)个结果

        # 遍历添加所有可能结果
        for i in range(num_orig_hyps):#这里建议取第一个就好了
            ...
            for j in range(params['beam_size'] * 2): #这里的分裂有和上面的2*beam sezi对应起来了
                # 构造可能的情况
                new_hyp = h.extend(token=top_k_ids[0, j].numpy(),
