    ...
    def call(self, dec_hidden, enc_output, dec_inp,
        ...
        for t in range(dec_inp.shape[1]):
            ...
            context_vector, attn, coverage_ret = self.attention(dec_hidden,
                                                                ...
                                                                use_coverage,
                                                                coverage_ret)
            #这里和上面的那个attention有啥区别，上面只是做了一次初始化，为了拿到encoder出来的context_vector
...
def _calc_final_dist(_enc_batch_extend_vocab, vocab_dists, attn_dists, p_gens, batch_oov_len, vocab_size, batch_size):
    ...
    #vocab_dists的尺寸为 (batch_size, vsize)，这里的拼接只是在vsize的大小上做了延长，维度是没有变化
    # vocab_dists = [0.002,0.0012,.....,0.006(he)，0,0,0]长度是30000，一定等于词表的长度
    # attn_dists = [0,002，0.0034,....,0.007(he)，chris(0.002),0.001,0.00001]长度肯定是30003，
    # final_dist = vocab_dists + attn_dists
