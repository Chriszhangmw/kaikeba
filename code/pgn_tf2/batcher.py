...
def article_to_ids(article_words, vocab):
    ...
    for w in article_words:
        i = vocab.word_to_id(w)#与词表做对比，如果没有包含在词表中，id设为unknown
        if i == unk_id:  # If w is OOV
            if w not in oovs:  # Add to list of OOVs
                oovs.append(w)
            oov_num = oovs.index(w)  # This is 0 for the first article OOV, 1 for the second article OOV...
            #oov[就将计就计，急急急急急急，斤斤计较斤斤计较]
            ids.append(vocab.size() + oov_num)  # This is e.g. 50000 for the first article OOV, 50001 for the second...
        else:
            ids.append(i)
    return ids, oovs #返回的对象，一个是包含了加长编码的unknown词，一个是所有oov的列表
...
def abstract_to_ids(abstract_words, vocab, article_oovs):
    ...
    for w in abstract_words:
        ...
        if i == unk_id:  # If w is an OOV word
            if w in article_oovs:  # If w is an in-article OOV这里相当于给摘要的oov词语按照输入来做编码，而不是一味的设置成为unknown
...
def get_dec_inp_targ_seqs(sequence, max_len, start_id, stop_id,vocab):
    """
    ...
    while len(inp) < max_len:
        inp.append(vocab.word2id[vocab.PAD_TOKEN])
    return inp, target


def get_enc_inp_targ_seqs(sequence, max_len, start_id, stop_id,vocab):
    """
    Given the reference summary as a sequence of tokens, return the input sequence for the decoder, and the target sequence which we will use to calculate loss. The sequence will be truncated if it is longer than max_len. The input sequence must start with the start_id and the target sequence must end with the stop_id (but not if it's been truncated).
    Args:
      sequence: List of ids (integers)
...
def example_generator(params, vocab, max_enc_len, max_dec_len, mode, batch_size):
    if mode == "train" or mode == 'eval':
        ...
        for raw_record in train_dataset:
            article = raw_record[0].numpy().decode("utf-8")#输入
            ...
            enc_input = get_enc_inp_targ_seqs(enc_input, max_enc_len, start_decoding, stop_decoding,vocab)
            enc_input_extend_vocab = get_enc_inp_targ_seqs(enc_input_extend_vocab,
                                                           max_enc_len, start_decoding,
                                                           stop_decoding,vocab)
            #enc_input， enc_input_extend_vocab这两个输入的区别是一个包含了oov的索引的输入，一个没有
            # mark长度
            ...
            dec_input, target = get_dec_inp_targ_seqs(abs_ids, max_dec_len, start_decoding, stop_decoding,vocab)


            abs_ids_extend_vocab = abstract_to_ids(abstract_words, vocab, article_oovs)
            _, target = get_dec_inp_targ_seqs(abs_ids_extend_vocab, max_dec_len, start_decoding, stop_decoding,vocab)
